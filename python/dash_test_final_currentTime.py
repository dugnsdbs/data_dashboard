...
arg = sys.argv
# make sure you put time year, month, day, hour, minutes, and seccond as arg
before_ninety = arg[1]
before_thirty = arg[2]
...
# make sure you put time year, month, day, hour, minutes, and seccond as arg put pacific time
def main():
    utc = pytz.timezone("UTC")
    querytimeFormat = "%Y-%m-%dT%H:%M:%SZ"
    timeFormat = "%Y%m%d%H%M%S"
    ninety_before = datetime.strptime(before_ninety, timeFormat)
    thirty_before = datetime.strptime(before_thirty, timeFormat)

    dt_start = ninety_before.astimezone(utc).strftime(querytimeFormat)
    dt_end = thirty_before.astimezone(utc).strftime(querytimeFormat)
...
def proccessor(dt_start, dt_end, ninety_before_pacific_to_utc, client):
    ...
    # timestampTime ===> eastern time before 90 minutes
